...
### Discharged Client (Churned)
A client that has been discharged in SimplePractice.

### Churn Rate
The percentage of clients discharged out of total clients during the time period.
...
### Rebook Rate
Of the active clients, this is the percentage that have their next appointment scheduled.
...
### Late Cancel
A cancellation made within the defined late cancellation window (self-evident based on practice policy).

### No-Show
Client did not attend the session without canceling (self-evident).
...
### Retention Rate
This is the percentage of clients with an intake during the time frame that stayed for 8 or more billable sessions. Note, retention is a metric that requires a longer time frame to calculate, as it takes time to see whether a client will continue to return. For this reason, retention has a grace period of 8 weeks. These clients would not have had time to complete 8 sessions, and so are excluded from the retention calculation. Visit the Retention Report to view different session-levels for retention.
