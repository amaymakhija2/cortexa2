...
// =============================================================================
// CLINICIAN BREAKDOWN PAGE
// =============================================================================
// Full-page view showing a clinician × month client count matrix.
// Displays every clinician on the y-axis and months on the x-axis, with
// active client counts in each cell. Hover to see the goal for that month.
// =============================================================================
...
// Generate monthly goals for a clinician (goals can change over time)
function generateMonthlyGoals(clinicianId: string, baseGoal: number, totalMonths: number): number[] {
  ...
  for (let month = 0; month < totalMonths; month++) {
    ...
    const growthFactor = Math.floor(month / 4) * 2; // +2 every 4 months
    const adjustment = ((seed + month) % 3) - 1; // -1, 0, or +1 random adjustment

    const goal = baseGoal + growthFactor + adjustment;
    goals.push(Math.max(10, goal)); // Minimum goal of 10
  }

  return goals;
}

// Generate client count data for a clinician across months
function generateClientData(clinicianId: string, clientGoal: number, totalMonths: number): number[] {
  const seed = parseInt(clinicianId) * 1000;
  const clients: number[] = [];

  const baseClients = clientGoal;

  for (let month = 0; month < totalMonths; month++) {
    const seasonalFactor = Math.sin((month + seed) * 0.5) * 3;
    const trendFactor = (month - totalMonths / 2) * 0.15;
    const randomVariance = ((seed * (month + 1) * 7) % 7) - 3;

    const count = Math.round(baseClients + seasonalFactor + trendFactor + randomVariance);
    clients.push(Math.max(5, count));
  }

  return clients;
}

// Get cell background color based on client count relative to goal
function getCellColor(clients: number, goal: number): string {
  const percentage = (clients / goal) * 100;
...
// Get cell text color based on client count relative to goal
function getCellTextColor(clients: number, goal: number): string {
  const percentage = (clients / goal) * 100;
...
export const ClinicianBreakdownPage: React.FC = () => {
  ...
  const allClinicianData = useMemo(() => {
    return CLINICIANS.filter(c => c.isActive).map(clinician => ({
      ...clinician,
      clients: generateClientData(clinician.id, clinician.clientGoal, totalMonths),
      goals: generateMonthlyGoals(clinician.id, clinician.clientGoal, totalMonths),
    }));
  }, []);
  ...
  const monthlyTotals = useMemo(() => {
    return monthLabels.map((_, monthIdx) =>
      clinicianData.reduce((sum, clinician) => sum + clinician.clients[monthIdx], 0)
    );
        ...
        <div className="px-6 sm:px-8 lg:pl-[100px] lg:pr-12 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button
                onClick={() => navigate('/practice-analysis?tab=capacity-client')}
              ...
              <tbody>
                {clinicianData.length === 0 ? (
                  ...
                  const avgClients = Math.round(
                    clinician.clients.reduce((sum, c) => sum + c, 0) / clinician.clients.length
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      {clinician.clients.map((clients, monthIdx) => {
                        const goal = clinician.goals[monthIdx];
                        const monthTotal = monthlyTotals[monthIdx];
                        const percentage = monthTotal > 0 ? (clients / monthTotal) * 100 : 0;
                        ...
                        return (
                          ...
                          >
                            {showPercentage ? (
                              <div
                                className="w-10 h-8 mx-auto rounded flex items-center justify-center text-sm font-semibold cursor-default bg-stone-100 text-stone-700"
                              >
                              ...
                              <div
                                className={`
                                  ...
                                  text-sm font-semibold cursor-default transition-transform hover:scale-110
                                  ${getCellColor(clients, goal)}
                                  ${getCellTextColor(clients, goal)}
                                `}
                                onMouseEnter={(e) => handleCellMouseEnter(e, clinician.id, monthIdx, clients, goal)}
                      ...
                      <td className="px-3 py-2.5 text-center border-l border-stone-200 bg-stone-100/50">
                        {showPercentage ? (
                          <span className="text-sm font-bold text-stone-800">
                            {Math.round((clinician.clients.reduce((sum, c) => sum + c, 0) / grandTotal) * 100)}%
                          </span>
                        ) : (
                          <span className={`text-sm font-bold ${
                            avgClients >= avgGoal ? 'text-emerald-700' : 'text-stone-800'
                          }`}>
                            {avgClients}
                          </span>
                        )}
                  ...
                  {monthlyTotals.map((total, monthIdx) => (
                    <td key={monthIdx} className="px-2 py-3 text-center">
                      <span className="text-sm font-bold text-stone-700">
                        {showPercentage ? '100%' : total}
                      </span>
                    </td>
                  ))}
                  <td className="px-3 py-3 text-center border-l border-stone-200 bg-stone-200/50">
                    <span className="text-sm font-bold text-stone-800">
                      {showPercentage ? '100%' : Math.round(grandTotal / totalMonths)}
                    </span>
                  </td>
                </tr>
              </tfoot>
      ...
      {tooltip && !showPercentage && (
        ...
        >
          <div className="flex items-center gap-2">
            <span>{tooltip.clients}</span>
            <span className="text-stone-400">/</span>
            <span className="text-stone-300">{tooltip.goal} goal</span>
            <span className="text-stone-400">·</span>
            <span className={
              (tooltip.clients / tooltip.goal) >= 1 ? 'text-emerald-400' :
              (tooltip.clients / tooltip.goal) >= 0.9 ? 'text-emerald-300' :
              (tooltip.clients / tooltip.goal) >= 0.7 ? 'text-amber-400' : 'text-rose-400'
            }>
              {Math.round((tooltip.clients / tooltip.goal) * 100)}%
            </span>
          </div>
          {/* Arrow */}
          <div
...
export default ClinicianBreakdownPage;
