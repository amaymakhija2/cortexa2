...
// =============================================================================
// SESSIONS BREAKDOWN PAGE
// =============================================================================
// Full-page view showing a clinician × month completed sessions matrix.
// Displays every clinician on the y-axis and months on the x-axis, with
// completed session counts in each cell. Hover to see the goal for that month.
// =============================================================================
...
// Generate monthly session goals for a clinician (goals can change over time)
function generateMonthlyGoals(clinicianId: string, baseGoal: number, totalMonths: number): number[] {
  ...
  for (let month = 0; month < totalMonths; month++) {
    ...
    const growthFactor = Math.floor(month / 4) * 5; // +5 every 4 months
    const adjustment = ((seed + month) % 3) - 1; // -1, 0, or +1 adjustment

    const goal = baseGoal + growthFactor + adjustment;
    goals.push(Math.max(20, goal)); // Minimum goal of 20
  }

  return goals;
}

// Generate session data for a clinician across months
function generateSessionData(clinicianId: string, sessionGoal: number, totalMonths: number): number[] {
  const seed = parseInt(clinicianId) * 1000;
  const sessions: number[] = [];

  const baseSessions = sessionGoal;

  for (let month = 0; month < totalMonths; month++) {
    const seasonalFactor = Math.sin((month + seed) * 0.5) * 10;
    const trendFactor = (month - totalMonths / 2) * 0.5;
    const randomVariance = ((seed * (month + 1) * 7) % 15) - 7;

    const count = Math.round(baseSessions + seasonalFactor + trendFactor + randomVariance);
    sessions.push(Math.max(15, count));
  }

  return sessions;
}

// Get cell background color based on sessions relative to goal
function getCellColor(sessions: number, goal: number): string {
  const percentage = (sessions / goal) * 100;
...
// Get cell text color based on sessions relative to goal
function getCellTextColor(sessions: number, goal: number): string {
  const percentage = (sessions / goal) * 100;
...
export const SessionsBreakdownPage: React.FC = () => {
  ...
  const allClinicianData = useMemo(() => {
    return CLINICIANS.filter(c => c.isActive).map(clinician => ({
      ...clinician,
      sessions: generateSessionData(clinician.id, clinician.sessionGoal, totalMonths),
      goals: generateMonthlyGoals(clinician.id, clinician.sessionGoal, totalMonths),
    }));
  }, []);
  ...
  const monthlyTotals = useMemo(() => {
    return monthLabels.map((_, monthIdx) =>
      clinicianData.reduce((sum, clinician) => sum + clinician.sessions[monthIdx], 0)
    );
              ...
              <tbody>
                {clinicianData.length === 0 ? (
                  ...
                  const avgSessions = Math.round(
                    clinician.sessions.reduce((sum, s) => sum + s, 0) / clinician.sessions.length
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      {clinician.sessions.map((sessions, monthIdx) => {
                        const goal = clinician.goals[monthIdx];
                        const monthTotal = monthlyTotals[monthIdx];
                        const percentage = monthTotal > 0 ? (sessions / monthTotal) * 100 : 0;
                            ...
                            {showPercentage ? (
                              ...
                              <div
                                className={`
                                  ...
                                  ${getCellColor(sessions, goal)}
                                  ${getCellTextColor(sessions, goal)}
                                `}
                                onMouseEnter={(e) => handleCellMouseEnter(e, clinician.id, monthIdx, sessions, goal)}
                      ...
                      <td className="px-3 py-2.5 text-center border-l border-stone-200 bg-stone-100/50">
                        {showPercentage ? (
                          <span className="text-sm font-bold text-stone-800">
                            {Math.round((clinician.sessions.reduce((sum, s) => sum + s, 0) / grandTotal) * 100)}%
                          </span>
                        ) : (
                          <span className={`text-sm font-bold ${
                            avgSessions >= avgGoal ? 'text-emerald-700' : 'text-stone-800'
                          }`}>
                            {avgSessions}
                          </span>
                        )}
      ...
      {tooltip && !showPercentage && (
        ...
        >
          <div className="flex items-center gap-2">
            <span>{tooltip.sessions}</span>
            <span className="text-stone-400">/</span>
            <span className="text-stone-300">{tooltip.goal} goal</span>
            <span className="text-stone-400">·</span>
            <span className={
              (tooltip.sessions / tooltip.goal) >= 1 ? 'text-emerald-400' :
              (tooltip.sessions / tooltip.goal) >= 0.9 ? 'text-emerald-300' :
              (tooltip.sessions / tooltip.goal) >= 0.7 ? 'text-amber-400' : 'text-rose-400'
            }>
              {Math.round((tooltip.sessions / tooltip.goal) * 100)}%
            </span>
          </div>
          {/* Arrow */}
          <div
...
export default SessionsBreakdownPage;
